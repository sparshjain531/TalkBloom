...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    if (!chat.groupChat) return next(new ErrorHandler("his is not a group chat", 400));

    if (chat.creator.toString() !== req.user.toString()) return next(new ErrorHandler("You are not allowed to add new Members", 403));
...
const renameGroup = TryCatch(async (req, res, next) => {
    ...
    if (chat.creator.toString() !== req.user.toString()) return next(
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    if (chat.groupChat && chat.creator.toString() !== req.user.toString()) return next(
    ...
    messageWithAttachments.forEach(({ attachments }) => attachments.forEach(({ public_ids }) => public_ids.push(public_ids)));
